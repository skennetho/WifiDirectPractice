...
import com.example.findwhousesmyhotspot.databinding.WatchBinding
...
class watchActivity : AppCompatActivity() {
    ...
    // 안드로이드 UI에 관련된 부분
    private lateinit var binding: WatchBinding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = DataBindingUtil.setContentView(this, R.layout.watch)
    ...
    private fun updateView(isConnected: Boolean) {
        ...
        // 로컬뷰의 이미지뷰 해당 이미지 끄기
        for (j in 0 until binding.layoutRemote.childCount) {
            val view = binding.layoutRemote.getChildAt(j)
